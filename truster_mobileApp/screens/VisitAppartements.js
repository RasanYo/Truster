...
import {Text, View, StyleSheet, TouchableWithoutFeedback, Keyboard}
  from 'react-native';
...
export default function VisitAppartments({navigation, route}) {
  ...
  const handleSelection = (_data, details) => {
    ...
    user.getPublicPosts(radius, [geo.lat, geo.lng], false, limit(queryState.limit)).then((res) => {
      setMapResults(res);
      const data = res.map((resDoc) => {
        return resDoc.data();
      });
      const lastVisible = data[data.length -1].geohash;
  ...
  const retrieveMore = () => {
    ...
    try {
      user.getPublicPosts(
        ...
        if (data.length) {
          const lastVisible = data[data.length -1].id;
          setQueryState({
            posts: data,
            lastVisible: lastVisible,
            loading: false,
  ...
  return (
    <View style={{flex: 1}} onPress={() => setInputClicked(false)}>
      <View style={styles.mainContainer} onPress={() => {
        Keyboard.dismiss; setInputClicked(false);
      }}>
        ...
        <View style={{marginTop: 20, flex: 1, zIndex: -100}} onPress={() => {
          Keyboard.dismiss; setInputClicked(false);
        }}>
